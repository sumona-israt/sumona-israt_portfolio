...
export default function GlyphPortal({
  ...
  const characters = Array.from(text).reduce<{ char: string; index: number }[]>((acc, char) => {
    const index = acc.length ? acc[acc.length - 1].index + acc[acc.length - 1].char.length : 0;
    acc.push({ char, index });
    return acc;
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/purity -- runs inside the useLayoutEffect callback (a side effect), not during render
